...
1. **API layer** (`src/gogmail/gog_api.py`, `src/gogmail/gemini_api.py`)
   ...
   - `GeminiAPI` wraps blocking `requests.post` in `asyncio.to_thread`. Both single-turn and multi-turn paths funnel through one `_call_sync(contents, system_instruction)`. `transcribe_audio(bytes)` sends inline audio to the same path for push-to-talk voice input.
   - **Voice** (`src/gogmail/voice.py`): push-to-talk records the mic by shelling out to the first available recorder (`pw-record`/`arecord`/`ffmpeg`/`sox`-`rec`) into a 16 kHz mono WAV; optional spoken replies shell out to a TTS engine (`espeak-ng`/`spd-say`/`say`). No native audio dependency. Gated by the `voice_input`/`spoken_replies` settings; the AI panel's mic button records, then feeds the Gemini transcript through the normal `submit_prompt` → tool-calling loop.
